...
function scales_chords_api_onload() {
    ...
    for (var i = 0; i < x.length; i++) {
        ...
        ajaxCall(api_url, params, function(xmlhttp) {
            ...
            if (scales_chords_api_debug) { document.getElementById(objid).innerHTML = "<!--" + mystring + "-->"; if (myStringArray[2].length > 0) document.getElementById(objid).innerHTML += myStringArray[2]; } else {
...
if (window.addEventListener) { window.addEventListener('load', scales_chords_api_onload, false); } else { window.attachEvent('onload', scales_chords_api_onload); }
...
function ajaxCall(url, params, successCallback, failCallback, ongoingCallback, cfg) {
    ...
    xhr.onreadystatechange = function() { if (xhr.readyState === 4) { if (xhr.status === 200) { if (successCallback) { successCallback(xhr); } } else { if (failCallback) { failCallback(xhr); } } } else { if (ongoingCallback) { ongoingCallback(xhr); } } };
    var paramString = "";
...
export { playAudio3, audioStop, scales_chords_api_onload }